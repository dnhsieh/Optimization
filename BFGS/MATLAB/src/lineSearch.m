...
%LINESEARCH was modified from HANSO 2.0 (https://cs.nyu.edu/~overton/software/hanso/)
%
...
if ( slpNow > -eps )
	...
	warning('Not a descent direction. Quit BFGS.');
...
if isinf(relRgt)
	warning(['Line search failed to bracket a point satisfying weak Wolfe conditions. ' ...
	         'Function may be unbounded below. Quit BFGS.']);
	stpTry = relTry * dirLen;
	sucFlg = false;
else
	warning(['Line search failed to satisfy weak Wolfe conditions, although ' ...
	         'a point satisfying conditions was bracketed in [%e, %e]. Quit BFGS.'], relLft, relRgt)
	stpTry = relTry * dirLen;
	sucFlg = false;
end
